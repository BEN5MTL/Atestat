...
if (quizCheckBtn) {
    quizCheckBtn.addEventListener('click', () => {
        ...
        let score = 0;
        if (q1.value === '95') score += 1;
        if (q2.value === '92') score += 1;
...
if (feedbackForm) {
    feedbackForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        feedbackInfo.textContent = 'Se trimite mesajul...';
        feedbackInfo.style.color = '#1f2a36';
        ...
        try {
            ...
            if (response.ok) {
                feedbackInfo.textContent = 'Mulțumim! Mesajul tău a fost trimis cu succes.';
                feedbackInfo.style.color = '#38a169';
                feedbackForm.reset();
            } else {
                throw new Error('Eroare la trimitere');
            }
        } catch (error) {
            feedbackInfo.textContent = 'Eroare la trimitere. Încearcă din nou.';
            feedbackInfo.style.color = '#cf0d0d';
        }

        setTimeout(() => { feedbackInfo.textContent = ''; }, 5000);
    });
}